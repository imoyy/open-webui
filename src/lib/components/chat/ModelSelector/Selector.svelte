...
>
	...
	>
		<div
			class="flex w-full text-left px-0.5 outline-none bg-transparent truncate {triggerClassName} justify-between text-black dark:text-gray-400 font-medium placeholder-gray-400 focus:outline-none"
		>
			{#if selectedModel}
				{selectedModel.label}
			{:else}
	...
	<DropdownMenu.Content
		class=" z-40 {$mobile
			? `w-full`
			: `${className}`} border dark:border-gray-300 drop-shadow-sm max-w-[calc(100vw-1rem)] justify-start rounded-xl  bg-white dark:bg-dark-700 dark:text-white shadow-lg  outline-none"
